...
function createRadialPercentSVG({
  percent,
  size = 170,
  stroke = 12,
  trackColor = '#99a3a6',
...
function observeAndAnimateRadial(
  containerEl,
  { threshold = 0.35, duration = 1000 } = {},
) {
  if (!containerEl) {
    return;
  }
  ...
  const io = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        const svg = entry.target.querySelector('svg.radial-svg');
        if (svg) {
...
function renderNames(data, container) {
  ...
  const list = document.createElement('ul');
  list.className = 'we-know-success-names-list';
  ...
  if (container.parentElement) {
    container.parentElement.classList.toggle('is-two-columns', names.length > 5);
  }
}
...
function getWorkingPercentage(params) {
  return callKnowSuccess('getAfterAU', params)
    .then((data) => data.WORKINGALL);
}

function getInternshipPercentage(params) {
...
export default async function decorate(block) {
  ...
  async function render() {
    ...
    if (percentWork > 0) {
      ...
      observeAndAnimateRadial(workFigure, { duration: 1000 });
    } else {
      workLabel.textContent = noDataMessage;
      workFigure.append(workLabel);
      figuresWrap.appendChild(workFigure);
    }
    ...
    if (level === 'UG') {
      ...
      if (percentInternships > 0) {
        ...
        observeAndAnimateRadial(internshipsFigure, { duration: 1000 });
      } else {
        internshipsLabel.textContent = noDataMessage;
        internshipsFigure.append(internshipsLabel);
        figuresWrap.appendChild(internshipsFigure);
        figuresWrap.classList.add('is-two-figures');
      }
    } else {
    ...
    namesListWrap.innerHTML = '';

    try {
      const data = await getNames(params);
      renderNames(data, namesListWrap);
    } catch (e) {
      // eslint-disable-next-line no-console
      console.error(e);
    }
